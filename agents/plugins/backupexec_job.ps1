...
foreach ($job in $Jobs) {
  ...
  $jobId = $job.JobID
  $jobHistory = Invoke-SQL "select TOP 1 JobName,FinalJobStatus,ActualStartTime,EndTime,ElapsedTimeSeconds,TotalDataSizeBytes,TotalRateMbMin,DDupRatio FROM vwJobHistorySummary2 WHERE (JobId = '$jobId' AND IsJobActive=0 ) ORDER BY ActualStartTime DESC"
...
if($jobHistoryResult.FinalJobStatus -eq 19)
 ...
 {
      $date = $jobHistoryResult.Endtime;
#      write-host "select TOP 1 AlertDate,Type FROM vwActiveAlerts WHERE (AlertDate = '$date')"
     $status = Invoke-SQL "select TOP 1 AlertDate,Type FROM vwActiveAlerts WHERE (AlertDate = '$date')"
     foreach ( $statusin in $status) { }
      if($statusin.Type)
      {
        # debug
        # write-host "report $jobname"
        write-host job $jobname $jobHistoryResult.FinalJobStatus $jobHistoryResult.Endtime $jobHistoryResult.ElapsedTimeSeconds $jobHistoryResult.TotalDataSizeBytes $jobHistoryResult.TotalRateMbMin $jobHistoryResult.DDupRatio
      }
      else
      {
        # debug
        # write-host "suprressed $jobname"
        write-host job $jobname 19 $jobHistoryResult.Endtime $jobHistoryResult.ElapsedTimeSeconds $jobHistoryResult.TotalDataSizeBytes $jobHistoryResult.TotalRateMbMin $jobHistoryResult.DDupRatio
      }     
 }
}
